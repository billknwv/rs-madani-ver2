...
@section('title', $clinic['name'] . ' - RS Madani')
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div class="animate-fade-in">
            <nav class="flex items-center gap-1.5 text-sm mb-6">
                ...
                <svg class="w-3.5 h-3.5 text-text-light/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <a href="{{ route('layanan.detail', 'rawat-jalan') }}" class="px-3 py-1.5 rounded-lg bg-white/5 text-text-light/60 hover:bg-white/10 hover:text-text-light transition-all duration-200">Rawat Jalan</a>
                <svg class="w-3.5 h-3.5 text-text-light/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="px-3 py-1.5 rounded-lg bg-accent/10 text-accent font-medium">{{ $clinic['name'] }}</span>
            </nav>
            <div class="flex items-start gap-5">
                <div class="w-1 h-20 bg-gradient-to-b from-accent to-accent/20 rounded-full shrink-0 mt-2"></div>
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 rounded-2xl bg-gradient-to-br {{ $clinic['color'] }} flex items-center justify-center text-white shadow-lg">
                        {!! $clinicIcons[$clinic['icon']] ?? $clinicIcons['heart'] !!}
                    </div>
                    <div>
                        <h1 class="text-3xl md:text-5xl font-bold text-text-light">{{ $clinic['name'] }}</h1>
                        <span class="inline-flex items-center gap-1.5 mt-2 px-3 py-1 rounded-full text-xs font-semibold bg-accent/10 text-accent">Rawat Jalan</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-10 lg:gap-12">
            <div class="lg:col-span-2">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 md:p-10 fade-in-section">
                    <h2 class="text-2xl font-bold text-primary mb-4">Tentang {{ $clinic['name'] }}</h2>
                    <p class="text-text-dark/60 leading-relaxed">{{ $clinic['description'] }}</p>
                    <div class="mt-6 flex flex-wrap gap-3">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-accent/10 text-accent">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            {{ $clinic['doctor_count'] }} Dokter
                        </span>
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-primary/10 text-primary">
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 fade-in-section">
                    <h3 class="text-sm font-bold text-primary uppercase tracking-wider mb-3">Jam Pelayanan</h3>
                    @if(count($clinic['schedule']))
                    <div class="space-y-2 text-sm text-text-dark/60">
                        @foreach(['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu','Minggu'] as $day)
                        <div class="flex justify-between py-0.5">
                            <span>{{ $day }}</span>
                            @if(isset($clinic['schedule'][$day]))
                            <span class="font-semibold text-primary">{{ $clinic['schedule'][$day] }}</span>
                            @else
                    ...
                    <div class="space-y-2 text-sm text-text-dark/60">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold text-primary">{{ $clinic['default_schedule']['weekday'] }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Sabtu</span>
                            <span class="font-semibold text-primary">{{ $clinic['default_schedule']['saturday'] }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Minggu / Libur</span>
                            <span class="font-semibold text-primary">{{ $clinic['default_schedule']['holiday'] }}</span>
                        </div>
                    </div>
                    @endif
                </div>
                <div class="bg-gradient-to-br from-accent to-[#e02e4f] rounded-2xl p-6 text-white fade-in-section">
                    ...
                    <p class="text-sm text-white/80 mb-4">Hubungi kami untuk informasi jadwal dokter atau pendaftaran.</p>
...
<section class="py-16 md:py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-4 mb-12 fade-in-section">
            <div class="w-14 h-14 rounded-2xl bg-gradient-to-br {{ $clinic['color'] }} flex items-center justify-center text-white shadow-lg">
                {!! $clinicIcons[$clinic['icon']] ?? $clinicIcons['heart'] !!}
            </div>
            <div>
                <h2 class="text-2xl md:text-3xl font-bold text-primary">Dokter {{ $clinic['name'] }}</h2>
                <p class="text-text-dark/50 text-sm mt-1">Dokter yang bertugas di {{ $clinic['name'] }}</p>
            </div>
        </div>
...
<section class="py-16 bg-primary relative overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center fade-in-section">
        ...
        <p class="text-text-light/70 max-w-2xl mx-auto mb-8 text-lg">Tim kami siap membantu Anda memilih jadwal dan dokter yang tepat sesuai kebutuhan.</p>
        ...
        <a href="{{ route('layanan.detail', 'rawat-jalan') }}" class="inline-flex items-center justify-center gap-2.5 bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-4 rounded-xl transition-all duration-300 ml-4">Kembali ke Rawat Jalan</a>
    </div>
</section>
@endsection
